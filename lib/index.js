...
var Client = (function () {
  ...
  Client.prototype.signup = function (signupInfo) {
    return this.http.post(this.url, signupInfo);
  };
  Client.prototype.verifyUser = function (id, code) {
    var s = this.url2 + '/' + id + '/' + code;
    return this.http.get(s);
  };
  Client.prototype.verifyUserAndSavePassword = function (id, code, password) {
    var s = this.url2 + '/' + id + '/' + code;
...
function signup(register, user, r, showMessage, showError, handleError, loading) {
  ...
  register(user).then(function (res) {
    var msg = getMessage(res.status, r);
